 ...
 * Manages pantry items, meal plan, current week, and recipes
 */
const AppState = {
    pantry: [],
    mealPlan: {},
    ...
    customRecipes: [],
    get recipes() {
        // Combine sample and custom recipes
        return [...this.sampleRecipes, ...this.customRecipes];
    }
};
...
function saveState() {
    try {
        ...
        localStorage.setItem('mealPlanner_customRecipes', JSON.stringify(AppState.customRecipes));
    } catch (e) {
...
function loadState() {
    try {
        ...
        if (mealPlanData) {
            ...
            // Migrate old format (just recipeId) to new format (object with recipeId and servings)
            AppState.mealPlan = {};
            Object.keys(parsed).forEach(key => {
                const value = parsed[key];
                if (typeof value === 'object' && value.recipeId !== undefined) {
                    // Already in new format
                    AppState.mealPlan[key] = value;
                } else {
                    // Old format - migrate to new format
                    AppState.mealPlan[key] = {
                        recipeId: value,
                        servings: 1
                    };
                }
        ...
        const customRecipesData = localStorage.getItem('mealPlanner_customRecipes');
        if (customRecipesData) {
            AppState.customRecipes = JSON.parse(customRecipesData);
            // Ensure all custom recipes have valid IDs
            let maxId = Math.max(...SAMPLE_RECIPES.map(r => r.id), 0);
            AppState.customRecipes.forEach(recipe => {
                if (!recipe.id || recipe.id <= maxId) {
                    maxId++;
                    recipe.id = maxId;
                } else {
                    maxId = Math.max(maxId, recipe.id);
                }
            });
        }
    } catch (e) {
...
function getNextRecipeId() {
    ...
    if (allRecipes.length === 0) return 1;
    return Math.max(...allRecipes.map(r => r.id)) + 1;
}
...
function validateRecipe(recipeData) {
    ...
    if (!recipeData.nutrition) {
        ...
        required.forEach(field => {
            if (recipeData.nutrition[field] === undefined || recipeData.nutrition[field] < 0) {
                errors.push(`Nutrition ${field} must be 0 or greater`);
            }
        });
    }
 ...
 * @returns {Object} {success: boolean, recipe: Object|null, errors: Array<string>}
 */
function addCustomRecipe(recipeData) {
    const validation = validateRecipe(recipeData);
    if (!validation.valid) {
        return { success: false, recipe: null, errors: validation.errors };
    }
    
    // Check for duplicate name in custom recipes
    const duplicate = AppState.customRecipes.find(r => 
        r.name.toLowerCase().trim() === recipeData.name.toLowerCase().trim()
    );
    if (duplicate) {
        return { success: false, recipe: null, errors: ['A recipe with this name already exists'] };
    }
    ...
    const newRecipe = {
        ...
        instructions: recipeData.instructions.map(inst => inst.trim()).filter(inst => inst.length > 0),
        servings: parseInt(recipeData.servings),
        nutrition: {
            calories: parseFloat(recipeData.nutrition.calories),
            protein: parseFloat(recipeData.nutrition.protein),
            carbs: parseFloat(recipeData.nutrition.carbs),
            fats: parseFloat(recipeData.nutrition.fats)
        },
    ...
    return { success: true, recipe: newRecipe, errors: [] };
}
 ...
 * @returns {Object} {success: boolean, recipe: Object|null, errors: Array<string>}
 */
function updateCustomRecipe(recipeId, recipeData) {
    const index = AppState.customRecipes.findIndex(r => r.id === recipeId);
    if (index === -1) {
        return { success: false, recipe: null, errors: ['Recipe not found'] };
    }
    ...
    const duplicate = AppState.customRecipes.find(r => 
        r.id !== recipeId && r.name.toLowerCase().trim() === recipeData.name.toLowerCase().trim()
    );
    if (duplicate) {
        return { success: false, recipe: null, errors: ['A recipe with this name already exists'] };
    }
    ...
    AppState.customRecipes[index] = updatedRecipe;
    saveState();
    
    return { success: true, recipe: updatedRecipe, errors: [] };
}
...
function deleteCustomRecipe(recipeId) {
    ...
    // Remove from meal plan if used
    Object.keys(AppState.mealPlan).forEach(key => {
        const mealPlanEntry = AppState.mealPlan[key];
        if (mealPlanEntry) {
            const mealRecipeId = typeof mealPlanEntry === 'object' ? mealPlanEntry.recipeId : mealPlanEntry;
    ...
    AppState.customRecipes.splice(index, 1);
    saveState();
...
/**
 * Check if a recipe is custom
 * @param {Object} recipe - Recipe object
 * @returns {boolean} True if custom recipe
 */
function isCustomRecipe(recipe) {
    return recipe.isCustom === true || AppState.customRecipes.some(r => r.id === recipe.id);
}
...
function renderRecipes(recipes) {
    ...
    recipes.forEach(recipe => {
        ...
        const isCustom = isCustomRecipe(recipe);
        ...
        card.innerHTML = `
            <div class="recipe-card-header">
                <div class="recipe-card-name">${recipe.name}${isCustom ? ' <span class="custom-badge">Custom</span>' : ''}</div>
            ...
            <div class="recipe-card-actions" onclick="event.stopPropagation();">
                ${isCustom ? `
                <button class="btn-edit" onclick="openEditRecipeModal(${recipe.id})" title="Edit recipe">✏️ Edit</button>
                <button class="btn-delete" onclick="deleteRecipe(${recipe.id})" title="Delete recipe">🗑️ Delete</button>
                ` : `
                <button class="btn-duplicate" onclick="duplicateAndEditRecipe(${recipe.id})" title="Duplicate and edit recipe">📋 Duplicate & Edit</button>
                `}
            </div>
        `;
        
        // Add click handler for viewing recipe (but not on action buttons)
...
function showRecipeModal(recipe) {
    ...
    body.innerHTML = `
        <div class="recipe-detail">
            <h2>${recipe.name}${isCustom ? ' <span class="custom-badge">Custom</span>' : ''}</h2>
            ${!isCustom ? `
            <div style="margin-bottom: 15px; padding: 10px; background-color: #e3f2fd; border-radius: 4px; border-left: 3px solid #2196f3;">
                <p style="margin: 0; color: #1565c0; font-size: 0.9em;">
                    💡 This is a sample recipe. Click "Duplicate & Edit" on the recipe card to create your own editable version.
                </p>
            </div>
            ` : ''}
            <div class="recipe-detail-section">
                ...
                <div><strong>Servings:</strong> ${recipe.servings} ${recipe.servings === 1 ? 'serving' : 'servings'}</div>
                <div class="nutrition-info">
                    <strong>Nutrition per serving:</strong> 
                    ${recipe.nutrition.calories} cal, 
                    ${recipe.nutrition.protein}g protein, 
                    ${recipe.nutrition.carbs}g carbs, 
                    ${recipe.nutrition.fats}g fats
                </div>
                <div class="nutrition-info" style="margin-top: 8px; color: #7f8c8d; font-size: 0.9em;">
                    <strong>Total recipe nutrition:</strong> 
                    ${(recipe.nutrition.calories * recipe.servings).toFixed(0)} cal, 
                    ${(recipe.nutrition.protein * recipe.servings).toFixed(1)}g protein, 
                    ${(recipe.nutrition.carbs * recipe.servings).toFixed(1)}g carbs, 
                    ${(recipe.nutrition.fats * recipe.servings).toFixed(1)}g fats
                </div>
            </div>
            <div class="recipe-detail-section">
                <h3>Ingredients</h3>
...
/**
 * Open recipe form modal for adding a new recipe
 */
function openAddRecipeModal() {
    const modal = document.getElementById('recipe-form-modal');
    const body = document.getElementById('recipe-form-body');
    
    body.innerHTML = `
        <h2>Add Custom Recipe</h2>
        <form id="recipe-form" onsubmit="handleRecipeFormSubmit(event)">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="recipe-servings" min="1" value="4" required>
                </div>
                <div class="form-group">
                    <label for="recipe-prep-time">Prep Time (minutes) *</label>
                    <input type="number" id="recipe-prep-time" min="0" value="30" required>
                </div>
                <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div id="ingredients-list">
                    <div class="ingredient-row">
                        ...
                        <input type="number" class="ingredient-quantity" placeholder="Qty" step="0.1" min="0.1" required>
                        ...
                        <button type="button" class="btn-remove" onclick="removeIngredientRow(this)">Remove</button>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <div id="instructions-list">
                    <div class="instruction-row">
                        <textarea class="instruction-text" placeholder="Enter instruction step" rows="2" required></textarea>
                        <button type="button" class="btn-remove" onclick="removeInstructionRow(this)">Remove</button>
                    </div>
                </div>
                <button type="button" class="btn-secondary" onclick="addInstructionRow()">+ Add Instruction</button>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" id="nutrition-calories" min="0" step="0.1" value="0" required>
                    </div>
                    <div class="form-group">
                        <label for="nutrition-protein">Protein (g)</label>
                        <input type="number" id="nutrition-protein" min="0" step="0.1" value="0" required>
                    </div>
                    <div class="form-group">
                        <label for="nutrition-carbs">Carbs (g)</label>
                        <input type="number" id="nutrition-carbs" min="0" step="0.1" value="0" required>
                    </div>
                    <div class="form-group">
                        <label for="nutrition-fats">Fats (g)</label>
                        <input type="number" id="nutrition-fats" min="0" step="0.1" value="0" required>
                    </div>
                </div>
            </div>
            
            <div id="recipe-form-errors" class="form-errors"></div>
            
            <div class="form-actions">
                <button type="button" class="btn-secondary" onclick="closeRecipeFormModal()">Cancel</button>
...
/**
 * Duplicate a sample recipe as a custom recipe and open edit modal
 * @param {number} recipeId - Recipe ID to duplicate
 */
function duplicateAndEditRecipe(recipeId) {
    // Find the recipe (could be sample or custom)
    ...
    // Check if already a custom recipe
    if (isCustomRecipe(recipe)) {
        openEditRecipeModal(recipeId);
        return;
    }
    
    // Create a duplicate as a custom recipe
    const duplicateName = `${recipe.name} (Copy)`;
    
    // Check if duplicate name already exists, if so, add a number
    let finalName = duplicateName;
    let counter = 1;
    while (AppState.customRecipes.some(r => r.name === finalName)) {
        finalName = `${recipe.name} (Copy ${counter})`;
        counter++;
    }
    ...
    const duplicatedRecipe = {
        id: getNextRecipeId(),
        name: finalName,
        ingredients: JSON.parse(JSON.stringify(recipe.ingredients)), // Deep copy
        instructions: [...recipe.instructions], // Copy array
        servings: recipe.servings,
        nutrition: { ...recipe.nutrition }, // Copy object
        prepTime: recipe.prepTime,
        difficulty: recipe.difficulty,
        isCustom: true
    };
    
    AppState.customRecipes.push(duplicatedRecipe);
    saveState();
    
    // Open edit modal for the new custom recipe
    openEditRecipeModal(duplicatedRecipe.id);
...
/**
 * Open recipe form modal for editing an existing recipe
 * @param {number} recipeId - Recipe ID to edit
 */
function openEditRecipeModal(recipeId) {
    const recipe = AppState.customRecipes.find(r => r.id === recipeId);
    if (!recipe) {
        alert('Recipe not found. Only custom recipes can be edited.');
        return;
    }
    ...
    const ingredientsHtml = recipe.ingredients.map((ing, index) => `
        <div class="ingredient-row">
            <input type="text" class="ingredient-name" placeholder="Ingredient name" value="${ing.name}" required>
            <input type="number" class="ingredient-quantity" placeholder="Qty" step="0.1" min="0.1" value="${ing.quantity}" required>
            <select class="ingredient-unit" required>
                ...
                <option value="lbs" ${ing.unit === 'lbs' ? 'selected' : ''}>lbs</option>
                ...
                <option value="whole" ${ing.unit === 'whole' ? 'selected' : ''}>whole</option>
            </select>
            <button type="button" class="btn-remove" onclick="removeIngredientRow(this)">Remove</button>
        </div>
    `).join('');
    ...
    const instructionsHtml = recipe.instructions.map(inst => `
        <div class="instruction-row">
            <textarea class="instruction-text" placeholder="Enter instruction step" rows="2" required>${inst}</textarea>
            <button type="button" class="btn-remove" onclick="removeInstructionRow(this)">Remove</button>
        </div>
    `).join('');
    ...
    body.innerHTML = `
        <h2>Edit Recipe</h2>
        <form id="recipe-form" onsubmit="handleRecipeFormSubmit(event, ${recipeId})">
            <div class="form-group">
                <label for="recipe-name">Recipe Name *</label>
            ...
            <div class="form-row">
                <div class="form-group">
                    <label for="recipe-servings">Servings *</label>
                    <input type="number" id="recipe-servings" min="1" value="${recipe.servings}" required>
                </div>
                <div class="form-group">
                    <label for="recipe-prep-time">Prep Time (minutes) *</label>
                    <input type="number" id="recipe-prep-time" min="0" value="${recipe.prepTime}" required>
                </div>
                <div class="form-group">
                    <label for="recipe-difficulty">Difficulty *</label>
                    <select id="recipe-difficulty" required>
            ...
            <div class="form-group">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="nutrition-calories">Calories</label>
                        <input type="number" id="nutrition-calories" min="0" step="0.1" value="${recipe.nutrition.calories}" required>
                    </div>
                    <div class="form-group">
                        <label for="nutrition-protein">Protein (g)</label>
                        <input type="number" id="nutrition-protein" min="0" step="0.1" value="${recipe.nutrition.protein}" required>
                    </div>
                    <div class="form-group">
                        <label for="nutrition-carbs">Carbs (g)</label>
                        <input type="number" id="nutrition-carbs" min="0" step="0.1" value="${recipe.nutrition.carbs}" required>
                    </div>
                    <div class="form-group">
                        <label for="nutrition-fats">Fats (g)</label>
                        <input type="number" id="nutrition-fats" min="0" step="0.1" value="${recipe.nutrition.fats}" required>
                    </div>
                </div>
            </div>
...
/**
 * Add instruction row to recipe form
 */
...
/**
 * Remove instruction row from recipe form
 * @param {HTMLElement} button - Remove button element
 */
 ...
 * @param {number} recipeId - Optional recipe ID for editing
 */
function handleRecipeFormSubmit(event, recipeId = null) {
    event.preventDefault();
    
    const errorsDiv = document.getElementById('recipe-form-errors');
    errorsDiv.innerHTML = '';
    ...
    const recipeData = {
        ...
        nutrition: {
            ...
            fats: document.getElementById('nutrition-fats').value
        }
    };
    ...
    let result;
    if (recipeId) {
        result = updateCustomRecipe(recipeId, recipeData);
    } else {
        result = addCustomRecipe(recipeData);
    }
    
    if (result.success) {
        closeRecipeFormModal();
        renderRecipes(AppState.recipes);
        renderMealPlan(); // Update meal plan dropdowns
        updateNutrition();
    } else {
        // Display errors
        errorsDiv.innerHTML = `
...
function deleteRecipe(recipeId) {
    if (!confirm('Are you sure you want to delete this recipe? It will be removed from your meal plan if it\'s currently scheduled.')) {
        return;
    }
    
    if (deleteCustomRecipe(recipeId)) {
        renderRecipes(AppState.recipes);
        renderMealPlan(); // Update meal plan dropdowns
        updateNutrition();
    } else {
 ...
 */
function updateMealPlan(day, meal, recipeId, servings = 1) {
 ...
 */
function updateMealServings(day, meal, servings) {
    const key = `${day}_${meal}`;
    if (AppState.mealPlan[key]) {
        AppState.mealPlan[key].servings = Math.max(1, parseInt(servings) || 1);
        saveState();
        updateNutrition();
    }
}
...
function renderMealPlan() {
    ...
    recipeSelects.forEach(select => {
        ...
        const mealPlanEntry = AppState.mealPlan[key];
        if (mealPlanEntry) {
            // Handle both old format (just recipeId) and new format (object)
            const recipeId = typeof mealPlanEntry === 'object' ? mealPlanEntry.recipeId : mealPlanEntry;
            select.value = recipeId;
            
            // Update servings input
            const servingsInput = document.querySelector(`.servings-input[data-day="${day}"][data-meal="${meal}"]`);
            if (servingsInput) {
                const servings = typeof mealPlanEntry === 'object' ? mealPlanEntry.servings : 1;
                servingsInput.value = servings;
            }
        } else {
            // Clear servings input if no recipe selected
            const servingsInput = document.querySelector(`.servings-input[data-day="${day}"][data-meal="${meal}"]`);
            if (servingsInput) {
 ...
 * @returns {Object} Grocery list organized by ingredient with meal breakdown
 */
function generateGroceryList() {
    ...
    // Collect all ingredients from meal plan with meal tracking
    days.forEach(day => {
        meals.forEach(meal => {
            const key = `${day}_${meal}`;
            const mealPlanEntry = AppState.mealPlan[key];
            if (mealPlanEntry) {
                // Handle both old format (just recipeId) and new format (object)
                const recipeId = typeof mealPlanEntry === 'object' ? mealPlanEntry.recipeId : mealPlanEntry;
                const servings = typeof mealPlanEntry === 'object' ? (mealPlanEntry.servings || 1) : 1;
                    ...
                    recipe.ingredients.forEach(ing => {
                        ...
                        if (!groceryList[ingKey]) {
                            groceryList[ingKey] = {
                                ...
                                quantity: 0,
                                unit: ing.unit,
                                mealBreakdown: [] // Track which meals contribute
                            };
                        }
                        // Calculate quantity per serving, then multiply by meal servings
                        const quantityPerServing = ing.quantity / recipe.servings;
                        const quantityNeeded = quantityPerServing * servings;
                        
                        // Add quantity needed based on servings
                        groceryList[ingKey].quantity += quantityNeeded;
                        
                        // Track meal contribution
                        groceryList[ingKey].mealBreakdown.push({
                            day: day,
                            meal: meal,
                            recipe: recipe.name,
    ...
    // Note: We preserve mealBreakdown even after pantry subtraction
    AppState.pantry.forEach(pantryItem => {
        Object.keys(groceryList).forEach(ingKey => {
            ...
            if (ingredientsMatch(groceryItem.name, pantryItem.name)) {
                // Calculate original quantity before subtraction (for display purposes)
                const originalQuantity = groceryItem.quantity;
                ...
                // Store pantry deduction info for display
                if (!groceryItem.pantryDeductions) {
                    groceryItem.pantryDeductions = [];
                }
                groceryItem.pantryDeductions.push({
                    pantryItem: pantryItem.name,
                    quantityUsed: originalQuantity - groceryItem.quantity,
    ...
    Object.keys(grouped).sort().forEach(name => {
        ...
        categoryDiv.innerHTML = `
            ...
            ${items.map(item => {
                ...
                // Show meal breakdown if available
                if (item.mealBreakdown && item.mealBreakdown.length > 0) {
                    itemHtml += `
                <div class="grocery-meal-breakdown">
                    <div class="breakdown-header">📋 Used in:</div>`;
                    
                    item.mealBreakdown.forEach(breakdown => {
                        const dayName = breakdown.day.charAt(0).toUpperCase() + breakdown.day.slice(1);
                        const mealName = breakdown.meal.charAt(0).toUpperCase() + breakdown.meal.slice(1);
                        itemHtml += `
                    <div class="breakdown-item">
                        <span class="breakdown-meal">${dayName} ${mealName}</span>
                        <span class="breakdown-recipe">${breakdown.recipe}</span>
                        <span class="breakdown-servings">(${breakdown.mealServings} ${breakdown.mealServings === 1 ? 'serving' : 'servings'})</span>
                        <span class="breakdown-quantity">→ ${breakdown.quantityNeeded.toFixed(2)} ${item.unit}</span>
                    </div>`;
                    });
                ...
                // Show pantry deductions if available
                if (item.pantryDeductions && item.pantryDeductions.length > 0) {
                    const totalDeducted = item.pantryDeductions.reduce((sum, ded) => sum + ded.quantityUsed, 0);
                    const originalTotal = item.quantity + totalDeducted;
                    itemHtml += `
                <div class="grocery-pantry-deduction">
                    <span class="deduction-label">✅ From pantry:</span>
                    <span class="deduction-info">Used ${totalDeducted.toFixed(2)} ${item.unit} (${((totalDeducted / originalTotal) * 100).toFixed(0)}% of needed)</span>
                </div>`;
                }
    ...
    days.forEach(day => {
        meals.forEach(meal => {
            ...
            if (mealPlanEntry) {
                ...
                const recipe = AppState.recipes.find(r => r.id === recipeId);
                if (recipe) {
                    // Add nutrition per serving multiplied by number of servings
                    ...
                    totals.fats += recipe.nutrition.fats * servings;
                }
            }
        });
    });
    ...
    days.forEach(day => {
        ...
        meals.forEach(meal => {
            ...
            if (mealPlanEntry) {
                ...
                if (recipe) {
                    // Add nutrition per serving multiplied by number of servings
                    daily[day].calories += recipe.nutrition.calories * servings;
                    daily[day].protein += recipe.nutrition.protein * servings;
                    daily[day].carbs += recipe.nutrition.carbs * servings;
                    daily[day].fats += recipe.nutrition.fats * servings;
                    daily[day].meals.push({
                        ...
                        nutrition: {
                            ...
                            fats: recipe.nutrition.fats * servings
                        }
                    });
                }
            }
...
function updateNutrition() {
    ...
    const daysWithMeals = Object.values(daily).filter(day => day.calories > 0).length;
    const avgDaily = {
        calories: daysWithMeals > 0 ? weekly.calories / 7 : 0,
        protein: daysWithMeals > 0 ? weekly.protein / 7 : 0,
        carbs: daysWithMeals > 0 ? weekly.carbs / 7 : 0,
        fats: daysWithMeals > 0 ? weekly.fats / 7 : 0
    };
    ...
    // Update or create average daily display
    let avgDisplay = document.getElementById('avg-daily-nutrition');
    if (!avgDisplay) {
        const summary = document.querySelector('.nutrition-summary');
        avgDisplay = document.createElement('div');
        avgDisplay.id = 'avg-daily-nutrition';
        avgDisplay.className = 'avg-daily-nutrition';
        avgDisplay.innerHTML = `
            <h3>Average Daily Nutrition</h3>
            <div class="nutrition-stats">
                ...
                <div class="stat-card">
                    <div class="stat-label">Avg Protein/Day (g)</div>
                    <div class="stat-value" id="avg-protein">0</div>
                </div>
                <div class="stat-card">
                    <div class="stat-label">Avg Carbs/Day (g)</div>
                    <div class="stat-value" id="avg-carbs">0</div>
                </div>
                <div class="stat-card">
                    <div class="stat-label">Avg Fats/Day (g)</div>
                    <div class="stat-value" id="avg-fats">0</div>
                </div>
            </div>
        `;
        summary.appendChild(avgDisplay);
    }
    
    document.getElementById('avg-calories').textContent = avgDaily.calories.toFixed(0);
    document.getElementById('avg-protein').textContent = avgDaily.protein.toFixed(1);
    document.getElementById('avg-carbs').textContent = avgDaily.carbs.toFixed(1);
    document.getElementById('avg-fats').textContent = avgDaily.fats.toFixed(1);

    // Update daily breakdown with per-meal details
    ...
    days.forEach(day => {
        ...
        if (dayData.meals.length > 0) {
            mealBreakdown = '<div class="meal-breakdown">';
            dayData.meals.forEach(mealData => {
                const mealName = mealData.meal.charAt(0).toUpperCase() + mealData.meal.slice(1);
                mealBreakdown += `
                    <div class="meal-nutrition-item">
                        <span class="meal-name">${mealName}:</span>
                        <span class="meal-recipe">${mealData.recipe}</span>
                        <span class="meal-servings">(${mealData.servings} ${mealData.servings === 1 ? 'serving' : 'servings'})</span>
                        <div class="meal-nutrition-details">
                            ${mealData.nutrition.calories.toFixed(0)} cal, 
                            ${mealData.nutrition.protein.toFixed(1)}g protein, 
                            ${mealData.nutrition.carbs.toFixed(1)}g carbs, 
                            ${mealData.nutrition.fats.toFixed(1)}g fats
                        </div>
                    </div>
                `;
            });
...
function init() {
    ...
    // Recipes: Add custom recipe button
